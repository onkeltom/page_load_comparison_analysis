...
tp_data <- fread("disconnect-mozilla-speed-report_updated.csv", colClasses = list(character=3:25))
...
by_browser <- perf_timings %>% 
  group_by(Browser) %>% 
  summarize(means = mean(loadEventEnd))
  
ggplot(by_browser, aes(Browser, means)) + geom_bar(stat = "identity", width = 0.5) + scale_fill_manual(values=cbPalette)
...
## Analyzing stability of results through bootstrapping

The previously calculated mean is the result of averaging across all 2000 measurement per browser. However, sessions are usually shorter. One way to analyze the stability of the results is through bootstrapping. We select a random set of measurements per browser, calculate means per browser and store these. Then, we repeat this, let's say, 10,000 times, and plot distribution of differences between Chrome Incognito and Firefox Quantum Tracking Protection afterwards.

```{r bootstrap, message=FALSE, warning=FALSE}
# create empty variables to hold our bootstrapping data
bootstrap <- data.frame(Browser = character(), means = integer())
distance <- c()

# perform 500 repetitions of sample calculations
# this is better be done using boot(), looking forward to receive patches
for (x in 1:10000) {
  by_browser <- perf_timings %>% 
    group_by(Browser) %>% 
    sample_n(200) %>% 
  ...
  bootstrap <- bind_rows(bootstrap, by_browser)
  
  distance[x] <- by_browser$means[by_browser$Browser == "Chrome Incognito"] / by_browser$means[by_browser$Browser == "Firefox Quantum Private Browsing"]
}
```

We can then look at the results of the bootstrapping per browser.

```{r bs_browser}
ggplot(bootstrap, aes(means, fill=Browser)) + geom_histogram(position="identity", alpha = 0.7, binwidth = 100) + scale_fill_manual(values=cbPalette)
```

More interestingly, we can also plot the distributions of the difference between Chrome and Firefox Quantum.

```{r dist_browser}
qplot(distance, geom = "histogram", binwidth = 0.05)
```

To proof stability of our measurements, we can calculate the mean of this difference distribution again which gives us **`r round(mean(distance),1)`x** as mean difference of differences between Chrome Incognito and Firefox Quantum Private Browsing.
...
ggplot(long_loading, aes(x = Browser, y = perc, fill = page_load_time, label = perc)) + 
  ...
  labs(y = "Percentages of pages loaded", color = "Page load time", fill = "Page load time") + 
...
*performance.timing* gives you events along the page load process. Let's print these events in order of appearance during page load and look at differences between browsers, looking at [newsweek.com](http://Newsweek.com) and [nbcnews.com](http://Nbcnews.com) as an example:
...
perf_timings %>%
  select(-secureConnectionStart, -redirectStart, -redirectEnd, -unloadEventEnd, -unloadEventStart) %>%
  filter(Domain == "http://Nbcnews.com" & grepl("Incog|Private", Browser)) %>%
  gather(metric, time, -Domain, -Browser) %>%
  mutate(metric = factor(metric, levels = standard)) %>%
  na.omit() %>%
  ggplot(aes(metric, time, fill = Browser)) + geom_boxplot() +
  ...
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(y = "time after navigationStart in ms", x = "performance.timing events", title = "nbcnews.com")
```
